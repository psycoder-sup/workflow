...
| `implement-core` | doctrine | How to implement one ticket well — contract selection, CONTEXT/ADR discipline, method, stay-on-ticket, self-verify. Mode-agnostic: nothing about briefs, workers or reports, so `/implement` loads no orchestration text. `/implement-orc` adds its worker overlay from `implement-orc/WORKER.md` (brief layout, file boundary, git duties, IMPLEMENTER REPORT). Never invoked directly. |
